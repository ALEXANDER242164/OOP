...
public class AppointmentService {
    ...
    public AppointmentResponse create(AppointmentRequest req) {
        Patient patient = patientRepo.findById(req.getPatientId())
                .orElseThrow(() -> new RuntimeException("Paciente no encontrado con ID: " + req.getPatientId()));
        Therapist therapist = therapistRepo.findById(req.getTherapistId())
                .orElseThrow(() -> new RuntimeException("Terapeuta no encontrado con ID: " + req.getTherapistId()));
        Room room = roomRepo.findById(req.getRoomId())
                .orElseThrow(() -> new RuntimeException("Sala no encontrada con ID: " + req.getRoomId()));
    ...
    private AppointmentCalendarItem toCalendarItem(Appointment a) {
        return new AppointmentCalendarItem(
                a.getId(),
                a.getSessionType(),
                a.getPatient() != null ? a.getPatient().getNombre() + " " + a.getPatient().getApellido() : null,
